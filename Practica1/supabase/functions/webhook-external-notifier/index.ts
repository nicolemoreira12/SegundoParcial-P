...
🛍️ Product ID: \`${event.payload.product_id}\`
📊 Quantity: *${event.payload.quantity}*
💰 Total: *$${event.payload.total}*
📅 Fecha: ${new Date(event.payload.created_at).toLocaleString('es-ES')}

_Evento ID: ${event.event_id}_
      `.trim();
    } else if (event.event_type === 'product.created') {
      message = `
🆕 *Nuevo Producto Creado*

🏷️ Product ID: \`${event.payload.product_id}\`
📝 Nombre: *${event.payload.name || 'N/A'}*
💵 Precio: *$${event.payload.price || 0}*
📦 Stock: *${event.payload.stock || 0}*
📅 Fecha: ${new Date(event.payload.created_at).toLocaleString('es-ES')}
